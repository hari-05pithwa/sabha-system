...
const BalakImage = ({ src }) => {
  const [isLoaded, setIsLoaded] = useState(false);
  return (
    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl sm:rounded-[1.8rem] overflow-hidden ring-4 ring-slate-50 shadow-sm bg-slate-100 relative shrink-0">
      {!isLoaded && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-4 h-4 border-2 border-indigo-200 border-t-indigo-600 rounded-full animate-spin" />
        </div>
...
export default function KidsInfoPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 pt-8 pb-32 font-sans bg-[#FDFDFD] min-h-screen relative overflow-x-hidden">
      <header className="mb-6">
        <div className="flex items-center justify-between mb-6">
          <Link href="/" className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm border border-slate-100 text-slate-500">
            <ArrowLeft size={18} />
          </Link>
          <div className="text-right">
            <p className="text-indigo-600 font-black text-[9px] uppercase tracking-[0.2em]">{session?.user?.area || "Mandal"}</p>
        ...
        <div className="relative group">
          ...
          <input
            ...
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-10 pr-10 focus:bg-white focus:ring-4 focus:ring-indigo-500/5 outline-none font-bold text-slate-700 placeholder:text-slate-300 transition-all text-sm"
          />
          {searchQuery && (
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        <div className="bg-slate-900 rounded-[2rem] p-5 text-white">
          <p className="text-3xl font-black">{filteredBalaks.length}</p>
          <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Total Kids</p>
        </div>
        <div className="bg-white rounded-[2rem] p-5 border border-slate-100 shadow-sm">
          <Cake size={14} className="text-pink-500 mb-1" />
          {nextBirthdayBalak ? (
            <>
              <p className="text-[19px] font-black text-slate-900 leading-tight truncate">
                {nextBirthdayBalak.firstName.charAt(0).toUpperCase() + nextBirthdayBalak.firstName.slice(1).toLowerCase()}
              </p>
              <p className="text-[10px] font-bold text-indigo-600">{new Date(nextBirthdayBalak.dob).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}</p>
            </>
      ...
      <div className="space-y-4">
        <AnimatePresence mode="popLayout">
          {filteredBalaks.map((balak) => (
            <motion.div key={balak._id} {...simpleFade} className="bg-white rounded-[2.2rem] p-4 shadow-sm border border-slate-50 flex items-start gap-4">
              <div className="relative shrink-0">
                <BalakImage src={balak.photoUrl} />
                <div className="absolute -bottom-1 -right-1 bg-indigo-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded-lg border-2 border-white">{balak.standard}</div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-base font-black text-slate-900 truncate">{formatName(balak.firstName)} {formatName(balak.lastName)}</h3>
                  <span className="text-[9px] font-black text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded-md shrink-0">{calculateAge(balak.dob)} YRS</span>
                </div>
                
                <div className="space-y-1.5 mb-3">
                  {/* Date of Birth Display */}
                  <div className="flex items-center gap-1.5 text-slate-500">
                    <CalendarDays size={10} className="shrink-0 text-indigo-400" />
                    <p className="text-[10px] font-bold uppercase tracking-tight">
                      {balak.dob ? new Date(balak.dob).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "Not set"}
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-1.5 text-slate-400">
                    ...
                    <p className="text-[10px] font-semibold leading-tight line-clamp-2">{balak.address}</p>
                  </div>
                </div>

                {balak.mobileNumber !== "N/A" && (
                  <a href={`tel:${balak.mobileNumber}`} className="inline-flex items-center justify-center gap-2 bg-slate-900 text-white w-full py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest active:bg-indigo-600">
                    <Phone size={10} fill="white" /> Call Parent
                  </a>
                )}
              </div>
            </motion.div>
