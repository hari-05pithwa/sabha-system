...
export default function RegistrationPage() {
  ...
  const inputStyle =
    "w-full bg-slate-50/50 border-2 border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 outline-none transition-all placeholder:text-slate-300 text-sm appearance-none";
  const labelStyle =
    "text-[11px] font-bold uppercase tracking-[0.15em] text-slate-400 ml-4 mb-2 block";
  ...
  return (
    <motion.div
      ...
      className="min-h-screen bg-[#FDFDFD] px-6 pt-12 pb-24 font-sans relative overflow-hidden"
    >
      ...
      <header className="flex items-center justify-between mb-8 relative z-10">
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={() => router.back()}
          className="w-11 h-11 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-slate-100 text-slate-500"
        >
          <ArrowLeft size={20} />
        </motion.button>
        ...
        <div className="space-y-6">
          ...
          >
            {loading ? (
              ...
              <>
                <CheckCircle2 size={18} /> Register Balak
              </>
            )}
          </motion.button>
        </div>
