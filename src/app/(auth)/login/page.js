...
export default function LoginPage() {
  ...
  // router.replace removes the login page from browser history so back-gesture exits the app.
  useEffect(() => {
  ...
  // Prevent UI flickering during session check
  ...
  const containerVars = {
    ...
    visible: {
      ...
      scale: 1,
      y: 0,
      transition: { duration: 0.5, staggerChildren: 0.08, ease: [0.22, 1, 0.36, 1] },
    },
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#F8FAFF] p-6 font-sans overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[60%] h-[40%] bg-indigo-100/40 blur-[80px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[40%] bg-blue-100/40 blur-[80px] rounded-full" />
      </div>

      <motion.div variants={containerVars} initial="hidden" animate="visible" className="w-full max-w-[440px] will-change-transform">
        <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] border border-white p-8 md:p-12 relative overflow-hidden">
          <motion.div variants={itemVars} className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-indigo-500 to-violet-600 text-white rounded-2xl mb-5 shadow-lg shadow-indigo-200">
              <Lock size={28} strokeWidth={2.5} />
            </div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Karyakar Access</h1>
            <p className="text-slate-500 font-medium text-sm">Manage your Sabha records</p>
          </motion.div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <motion.div variants={itemVars} className="space-y-2">
              <label className="text-[12px] font-bold text-slate-400 uppercase tracking-[0.1em] ml-2">User ID</label>
              <div className="relative group">
                ...
                <input
                  type="text"
                  required
                  className="w-full pl-14 pr-7 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-indigo-500 outline-none transition-all font-semibold text-slate-700 text-[16px]" 
                  value={username}
                  onChange={(e) => setUsername(e.target.value)}
                  placeholder="Enter your ID"
                />
            ...
            <motion.div variants={itemVars} className="space-y-2">
              <label className="text-[12px] font-bold text-slate-400 uppercase tracking-[0.1em] ml-2">Password</label>
              <div className="relative group">
            ...
            <motion.div variants={itemVars} className="flex items-center ml-2">
              <button type="button" onClick={() => setRememberMe(!rememberMe)} className="flex items-center group cursor-pointer touch-none">
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${rememberMe ? 'bg-indigo-600 border-indigo-600' : 'bg-slate-50 border-slate-200'}`}>
                  {rememberMe && <Check size={14} className="text-white" strokeWidth={3} />}
                </div>
                <span className="ml-3 text-sm font-semibold text-slate-500">Remember my ID</span>
              </button>
            </motion.div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.96 }}
              disabled={loading}
              className="w-full bg-slate-900 text-white font-bold py-4 rounded-2xl shadow-xl disabled:opacity-70 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? <Loader2 className="animate-spin" size={22} /> : <span>Sign In <ChevronRight size={18} className="inline" /></span>}
            </motion.button>
          </form>
