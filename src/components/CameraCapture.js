...
export default function CameraCapture({ onUpload }) {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    const formData = new FormData();
    formData.append("file", file);
    // CRITICAL: Uses the preset from your .env (balak_photos)
    formData.append("upload_preset", process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET);
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        toast.success("Image captured successfully!", { duration: 1500 });
      } else {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4">
      {/* Hidden Input with Direct Camera Access */}
      <input
        type="file"
        id="cameraInput"
        accept="image/*"
      ...
      <div 
        ...
        className="relative w-32 h-32 bg-slate-50 rounded-full overflow-hidden border-4 border-white shadow-xl flex items-center justify-center cursor-pointer active:scale-95 transition-all"
      >
        {preview ? (
          <img src={preview} className="w-full h-full object-cover" alt="Preview" />
        ) : (
      ...
      <button
        type="button"
        onClick={() => document.getElementById('cameraInput').click()}
        className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-full border border-indigo-100 hover:bg-indigo-100 transition-colors"
      >
        {preview ? <RefreshCw size={14} /> : <Camera size={14} />}
